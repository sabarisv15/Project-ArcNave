...
async function raw(model) {
  ...
  const body = {
    model,
    messages: [
      { role: 'system', content: 'You are ARCNAVE, a campus assistant. Use the attendance_summary tool for attendance questions.' },
  ...
  console.log(`--- model=${model} status=${res.status} latencyMs=${latency} finish_reason=${json.choices && json.choices[0].finish_reason} ---`);
