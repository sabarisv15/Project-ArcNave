...
flowchart TD
    ...
    F -- yes --> G["Build it yourself as a custom tool:<br/>file ops, DB queries, memory, ERP calls"]
...
## 1.5 Confirmed configuration for this build

This is the actual stack decided for this assistant — not the full
general-purpose list above, the subset that earned its place for a
...
prompt, session mechanics) — implemented as custom backend/frontend code
against this system's own storage and UI, no third-party service involved.

**`web_search` / `web_fetch` implementation choice: Gemini API, not
built from scratch.** Rather than standing up a separate search pipeline,
these two are served through Gemini's API using its built-in
search-grounding capability — the model call itself returns grounded,
cited results. This replaces what would otherwise be one of the "needs an
external API" build items with a single existing API integration.

**Dropped entirely for this build:** the 6 vendor-platform-only tools
(plugin/skill catalogs, app recommendations, autonomous research), and
`places_search` / `weather_fetch` / `fetch_sports_data` / `image_search` —
none are relevant to a college-management domain.
...
flowchart TD
    ...
    B -- "current or live external info" --> C["web_search / web_fetch via Gemini API grounding"]
...
Rated using the same tiering logic as this build's own risk ladder — the
