...
async function geometryText(buffer) {
  ...
  for (let p = 1; p <= doc.numPages; p += 1) {
    ...
    buckets.sort((a, b) => b.y - a.y).forEach((b) => {
      lines.push(b.items.sort((p1, q) => p1.x - q.x).map((it) => it.str).join(' | '));
    });
  }
...
async function main() {
  console.log('file'.padEnd(38), 'pages'.padStart(6), 'flat ms'.padStart(9), 'geom ms'.padStart(9), 'ratio'.padStart(7));
  for (const name of FILES) {
    ...
    console.log(
      ' '.repeat(38),
      `chars: flat ${flat.text ? flat.text.length : 0}, geometry ${geom.text.length}`,
    );
  }
}
