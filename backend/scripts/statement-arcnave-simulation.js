...
function analyze(text, { filter, operation = 'count', comparison, identityPattern } = {}) {
  ...
  const { strategy, records, sections, coverage } = tableExtraction.extractRecords(text);
...
show(
  'A1. "total PLB credit" -> operation sum, pattern captures the amount',
  analyze(TEXT, { operation: 'sum', filter: { pattern: 'PLB[^\\n]*?([\\d,]+\\.\\d{2})' } }),
);

show(
  'A2. "how many PLB rows" -> operation count',
  analyze(TEXT, { operation: 'count', filter: { pattern: 'PLB', mode: 'include' } }),
);

show(
  'A3. "PLB rows over 0" -> operation compare',
  analyze(TEXT, {
    operation: 'compare',
...
const NO_LEGEND = TEXT.split('\n')
  ...
  .join('\n');
show(
  'B1. same call, after removing the 2 legend lines that contain " | "',
...
const oneRow = [
  {
    key: 'r1',
    cells: ['08.04.2025 R2 6282520739 PLB FOR MARCH 25 TN 0.000 12,109.37 0.00 0.00 3,390.63 0.00 15,500.00'],
  },
];
console.log(
  '  sum of "15,500.00" via operation sum :',
  JSON.stringify(aggregate.aggregate(oneRow, { operation: 'sum', filter: { pattern: '([\\d,]+\\.\\d{2})$' } })),
);
console.log(
  "  same value via compare's parseNumeric:",
  JSON.stringify(
    aggregate.compareRecords(oneRow, {
...
try {
  ...
  console.log('  groupBy: "category" ->', err.message);
}
try {
  aggregate.aggregate([{ key: 'a' }], { groupBy: 2, filter: { pattern: 'PLB' }, operation: 'count' });
} catch (err) {
  console.log('  groupBy: <column index> ->', err.message);
}
