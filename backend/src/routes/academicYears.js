...
function createAcademicYearsRouter() {
  ...
  router.post('/academic-years/:id/activate', requirePermission('academic_years.activate'), asyncHandler(async (req, res) => {
    if (!requireResolvedTenant(req, res)) return;
    try {
      const academicYear = await academicYearService.activateAcademicYear(req.dbClient, req.params.id, { actorUserId: identityService.resolveActorUserId(req.capabilities) });
      res.json(academicYear);
    } catch (err) {
      if (mapAcademicYearServiceError(err, res)) return;
      throw err;
  ...
  router.post('/academic-years/:id/complete', requirePermission('academic_years.complete'), asyncHandler(async (req, res) => {
    if (!requireResolvedTenant(req, res)) return;
    try {
      const academicYear = await academicYearService.completeAcademicYear(req.dbClient, req.params.id, { actorUserId: identityService.resolveActorUserId(req.capabilities) });
      res.json(academicYear);
    } catch (err) {
      if (mapAcademicYearServiceError(err, res)) return;
      throw err;
