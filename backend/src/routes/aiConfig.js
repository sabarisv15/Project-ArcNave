...
function createAiConfigRouter() {
  ...
  router.get('/ai-config/capabilities', requirePermission('ai_config.read'), asyncHandler(async (req, res) => {
    if (!requireResolvedTenant(req, res)) return;
  ...
  // already enforce.
  router.get('/ai-config/ops-status', requirePermission('ai_config.read'), asyncHandler(async (req, res) => {
    if (!requireResolvedTenant(req, res)) return;
    const {
      provider, config, fallbackProvider,
    } = await configurationService.getAiConfig(req.dbClient, req.collegeId);
