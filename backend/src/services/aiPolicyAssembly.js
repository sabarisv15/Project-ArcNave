...
const CORE = 'Never claim to have taken an action (sending a message, changing a record) that no tool actually '
...
const CONTINUITY = 'Treat the current message as a continuation of "Conversation so far" above, not a fresh '
...
const TOOL_SELECTION = 'Each tool is for a specific, narrow purpose (e.g. reading THIS college\'s own profile, '
...
const PLAN = 'Use the multi-step plan tool only for a genuine multi-step request that one tool alone cannot '
...
// AGENT_SYSTEM_PROMPT.
const FILE = 'NEVER tell the user you cannot produce a document, PDF, Word file, or download — you genuinely '
...
const ARTIFACT = "An open artifact's own content is the actual document a user is drafting — a chat reply alone "
...
const MODE_PREFIX = {
  ...
  general: "You are ARCNAVE's assistant, currently in Research mode — help with research, project work, "
