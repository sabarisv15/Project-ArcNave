...
async function searchAll(client, { query, actorUserId, actorRole, collegeId }) {
  ...
  const [students, staff, classes] = await Promise.all([
    studentService.listStudents(client, { limit: 500 }, { actorUserId, actorRole, collegeId })
      .then((rows) => rows.filter((s) => matches(q, [s.full_name, s.roll_no]))),
    ['principal', 'hod'].includes(actorRole)
      ? staffService.listStaffForActor(client, { actorUserId, actorRole, collegeId })
        .then((rows) => rows.filter((s) => matches(q, [s.full_name, s.staff_code])))
      : Promise.resolve([]),
    academicService.listClasses(client, { limit: 500 })
      .then((rows) => rows.filter((c) => matches(q, [c.class_name]))),
  ]);

  return {
    students: students.slice(0, RESULT_LIMIT).map((s) => ({
      entityType: 'student', id: s.id, label: s.full_name, meta: s.roll_no,
    })),
    staff: staff.slice(0, RESULT_LIMIT).map((s) => ({
      entityType: 'staff', id: s.id, label: s.full_name, meta: s.staff_code,
    })),
    classes: classes.slice(0, RESULT_LIMIT).map((c) => ({
      entityType: 'class', id: c.id, label: c.class_name, meta: null,
    })),
  };
}
