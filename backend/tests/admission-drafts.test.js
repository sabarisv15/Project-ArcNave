...
function requestRaw(baseUrl, path, method, { headers = {}, body, isJson = true } = {}) {
...
test('admission drafts', async (t) => {
  ...
  await t.test(
    'full draft lifecycle: create -> multipart upload+classify -> extract -> poll to completion -> auto-filled -> complete -> real student+documents',
    async () => {
      const token = await loginTutor();
      const auth = headersFor(token);

      const created = await requestRaw(baseUrl, '/api/v1/students/admission-drafts', 'POST', {
        headers: auth,
        body: {},
      });
      ...
      const docTypes = await requestRaw(baseUrl, '/api/v1/document-types?module=student_admission', 'GET', {
        headers: auth,
      });
      ...
      const extractResp = await requestRaw(baseUrl, `/api/v1/students/admission-drafts/${draftId}/extract`, 'POST', {
        headers: auth,
        body: {},
      });
      ...
      for (let i = 0; i < 40; i += 1) {
        ...
        // eslint-disable-next-line no-await-in-loop
        const poll = await requestRaw(baseUrl, `/api/v1/students/admission-drafts/${draftId}`, 'GET', {
          headers: auth,
        });
        state = poll.body;
      ...
      assert.ok(
        state.extractionJob,
        'extraction job should be visible via getDraft, not just the Principal-only /background-jobs route',
      );
      ...
      const patchResp = await requestRaw(baseUrl, `/api/v1/students/admission-drafts/${draftId}`, 'PATCH', {
        headers: auth,
        body: { rollNo: 'ADMTEST01', fullName: 'Admission Test Student' },
      });
      assert.equal(patchResp.status, 200);

      const completeResp = await requestRaw(baseUrl, `/api/v1/students/admission-drafts/${draftId}/complete`, 'POST', {
        headers: auth,
        body: {},
      });
      ...
      const documentsResp = await requestRaw(baseUrl, `/api/v1/documents?student_id=${studentId}`, 'GET', {
        headers: auth,
      });
  ...
  // regression here).
  await t.test(
    "a real failure inside buildExtractionHandler's first transaction rolls back cleanly — the pool stays healthy for the very next unrelated request (round 8 fix regression)",
    async () => {
      const token = await loginTutor();
      const auth = headersFor(token);
      ...
      try {
        ...
        for (let i = 0; i < 40; i += 1) {
          ...
          if (
            state.extractionJob &&
            state.extractionJob.status !== 'pending' &&
            state.extractionJob.status !== 'running'
          )
            break;
        }
      } finally {
      ...
      assert.equal(
        forcedFailureCalls,
        1,
        'the forced failure should have actually fired inside the handler, not been bypassed',
      );
      assert.ok(state.extractionJob);
      assert.equal(state.extractionJob.status, 'failed');

      const sanityResp = await requestRaw(baseUrl, `/api/v1/students/admission-drafts/${draftId}`, 'GET', {
        headers: auth,
      });
  ...
  await t.test(
    "creating a draft is rejected for tutoruser's personal login even though that person currently occupies the class's L4 seat",
    async () => {
      const token = await login('tutoruser');
      const resp = await requestRaw(baseUrl, '/api/v1/students/admission-drafts', 'POST', {
  ...
  await t.test('a draft cannot be read, updated, or completed by anyone other than its own creator', async () => {
    ...
    const getResp = await requestRaw(baseUrl, `/api/v1/students/admission-drafts/${draftId}`, 'GET', {
  ...
  await t.test(
    'DELETE removes an uploaded draft document; a second delete is a no-op; only the owner may delete',
    async () => {
      const token = await loginTutor();
      const auth = headersFor(token);
      ...
      const otherToken = await login('othertutor');
      const forbiddenResp = await requestRaw(
        baseUrl,
        `/api/v1/students/admission-drafts/${draftId}/documents/marksheet_10th`,
        'DELETE',
        {
          headers: headersFor(otherToken),
        },
      ...
      const deleteResp = await requestRaw(
        baseUrl,
        `/api/v1/students/admission-drafts/${draftId}/documents/marksheet_10th`,
        'DELETE',
        { headers: auth },
      ...
      const draftAfterDelete = await requestRaw(baseUrl, `/api/v1/students/admission-drafts/${draftId}`, 'GET', {
        headers: auth,
      });
      assert.equal(draftAfterDelete.status, 200);
      assert.equal(
        (draftAfterDelete.body.documents || []).length,
        0,
        'the document row must actually be gone, not just its file',
      );

      const secondDeleteResp = await requestRaw(
        baseUrl,
        `/api/v1/students/admission-drafts/${draftId}/documents/marksheet_10th`,
        'DELETE',
        { headers: auth },
      ...
      assert.equal(
        secondDeleteResp.status,
        204,
        'deleting an already-deleted (or never-uploaded) docType is a silent no-op, not an error',
      );
    },
  );
  ...
  await t.test('DELETE on a nonexistent draft is rejected (404, not a silent no-op)', async () => {
    ...
    const resp = await requestRaw(
      baseUrl,
      '/api/v1/students/admission-drafts/00000000-0000-0000-0000-000000000000/documents/marksheet_10th',
      'DELETE',
      {
        headers: headersFor(token),
      },
