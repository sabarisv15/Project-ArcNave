...
const {
  STABILITY, segment, buildContext, flattenToPrompts, contextFromFlatPrompts,
} = aiContextAssembly;

function seg(overrides) {
  return {
    source: 'x', stability: STABILITY.STATIC, target: 'system', content: 'content', ...overrides,
  };
}
...
test('segment(): valid input returns the same fields, no extras', () => {
  const s = segment(seg({
    source: 'identity', stability: STABILITY.CONVERSATION, target: 'system', content: 'Identity Context',
  }));
  assert.deepEqual(s, {
    source: 'identity', stability: STABILITY.CONVERSATION, target: 'system', content: 'Identity Context',
  });
...
test('buildContext(): preserves segment order, passes tools/images through untouched including undefined', () => {
  ...
  assert.deepEqual(ctx.segments.map((s) => s.source), ['a', 'b']);
...
test('flattenToPrompts: executeWorkflowPlan pattern (system: preamble+prefix+policy+plan-note+identity, user: tool-data+guidance)', () => {
  const ctx = buildContext([
    segment({
      source: 'safety-preamble', stability: STABILITY.STATIC, target: 'system', content: 'PREAMBLE',
    }),
    segment({
      source: 'mode-prefix', stability: STABILITY.STATIC, target: 'system', content: 'MODE_PREFIX',
    }),
    segment({
      source: 'policy-modules', stability: STABILITY.CONVERSATION, target: 'system', content: 'POLICY',
    }),
    segment({
      source: 'plan-summary-note', stability: STABILITY.TURN, target: 'system', content: 'This answer combines the results of 2 tool(s), run as one plan:\nt1: d1\nt2: d2',
    }),
    segment({
      source: 'identity', stability: STABILITY.CONVERSATION, target: 'system', content: 'IDENTITY',
    }),
    segment({
      source: 'tool-result-data', stability: STABILITY.VOLATILE, target: 'user', content: 'USER_DATA',
    }),
    segment({
      source: 'tool-result-answer-guidance', stability: STABILITY.STATIC, target: 'user', content: 'GUIDANCE',
    }),
  ]);
...
test('flattenToPrompts: askGeneralChat pattern — no safety-preamble segment at all, genuinely shorter list', () => {
  const ctx = buildContext([
    segment({
      source: 'mode-prefix', stability: STABILITY.STATIC, target: 'system', content: 'MODE_PREFIX.general',
    }),
    segment({
      source: 'policy-modules', stability: STABILITY.CONVERSATION, target: 'system', content: 'POLICY',
    }),
    ...
    segment({
      source: 'question', stability: STABILITY.TURN, target: 'user', content: 'promptQuestion',
    }),
  ]);
  const { systemPrompt, userPrompt } = flattenToPrompts(ctx);
...
test('flattenToPrompts: conditional image-unavailable-note segment, present vs. omitted (not empty-string)', () => {
  ...
  const withNote = flattenToPrompts(buildContext([
    ...base,
    segment({
      source: 'image-unavailable-note', stability: STABILITY.TURN, target: 'user', content: 'cannot view images',
    }),
  ]));
...
test('flattenToPrompts: tools/images pass through as context fields, never stringified into prompt text', () => {
  ...
  const ctx = buildContext([
    ...
    segment({
      source: 'question', stability: STABILITY.TURN, target: 'user', content: 'q',
    }),
  ], { tools, images });
...
test('fingerprint: deterministic — same segments produce the same fingerprint', () => {
  const build = () => buildContext([
    segment({
      source: 'core', stability: STABILITY.STATIC, target: 'system', content: 'CORE TEXT',
    }),
    segment({
      source: 'identity', stability: STABILITY.CONVERSATION, target: 'system', content: 'IDENTITY',
    }),
...
test('fingerprint: changes when a STATIC segment\'s content changes', () => {
  const a = buildContext([segment({
    source: 'core', stability: STABILITY.STATIC, target: 'system', content: 'CORE TEXT',
  })]);
  const b = buildContext([segment({
    source: 'core', stability: STABILITY.STATIC, target: 'system', content: 'CORE TEXT v2',
  })]);
...
test('fingerprint: changes when a CONVERSATION segment\'s content changes', () => {
  const a = buildContext([segment({
    source: 'identity', stability: STABILITY.CONVERSATION, target: 'system', content: 'Role: hod',
  })]);
  const b = buildContext([segment({
    source: 'identity', stability: STABILITY.CONVERSATION, target: 'system', content: 'Role: principal',
  })]);
  assert.notEqual(a.fingerprint, b.fingerprint);
});
...
test('fingerprint: UNCHANGED when only a TURN segment changes (the question)', () => {
  ...
  const a = buildContext([staticPart, segment({
    source: 'question', stability: STABILITY.TURN, target: 'user', content: 'what is the capital of France?',
  })]);
  const b = buildContext([staticPart, segment({
    source: 'question', stability: STABILITY.TURN, target: 'user', content: 'a totally different question entirely',
  })]);
...
test('fingerprint: UNCHANGED when only a VOLATILE segment changes (a timestamped tool result)', () => {
  ...
  const a = buildContext([staticPart, segment({
    source: 'tool-result-data', stability: STABILITY.VOLATILE, target: 'user', content: 'retrievedAt: 2026-08-23T00:00:00Z',
  })]);
  const b = buildContext([staticPart, segment({
    source: 'tool-result-data', stability: STABILITY.VOLATILE, target: 'user', content: 'retrievedAt: 2026-08-24T12:00:00Z',
  })]);
...
test('contextFromFlatPrompts: round-trips a flat {systemPrompt, userPrompt, tools, images} object through flattenToPrompts unchanged', () => {
  const flat = {
    systemPrompt: 'SYS', userPrompt: 'USER', tools: [{ name: 't' }], images: [{ mimeType: 'image/png', base64: 'x' }],
  };
