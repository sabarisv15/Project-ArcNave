...
test('getMonthlyTokenQuota', async (t) => {
  ...
  await t.test(
    'a configured 0 is honored (an explicit "cut this college off"), never treated as falsy-and-ignored',
    async () => {
...
test('getUsageStatus', async (t) => {
  ...
  await t.test('reports withinLimit: false once calls-in-window reach the rate limit', async () => {
    ...
    mockUsageWindow(t, {
      periodTokens: 0,
      periodCallCount: globalConfig.aiRateLimitPerMinute,
      windowCallCount: globalConfig.aiRateLimitPerMinute,
    });
...
test('checkUsageLimits', async (t) => {
  ...
  await t.test('throws AiRateLimitExceededError when under quota but over the per-minute rate limit', async () => {
    ...
    mockUsageWindow(t, {
      periodTokens: 10,
      periodCallCount: 40,
      windowCallCount: globalConfig.aiRateLimitPerMinute + 5,
    });
    await assert.rejects(
      () => aiCostControlService.checkUsageLimits({}, 'c1'),
  ...
  await t.test(
    'quota is checked before rate limit — an over-quota-AND-over-rate-limit college gets the quota error',
    async () => {
      mockQuotaConfig(t, { monthlyTokenQuota: 100 });
      mockUsageWindow(t, {
        periodTokens: 500,
