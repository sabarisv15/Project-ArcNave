...
test('aiService.askAgent: unconfigured LLM provider throws LlmNotConfiguredError, no tool ever runs', async () => {
  ...
  // Seven queries ran before the LLM call itself failed: askAgent's own
  ...
  // own 'ai_quota' configurations lookup, then (ARCNAVE modernization P2
  // / PDF D4) its two now-independent reads: the incremental
  // ai_usage_counters PK lookup (monthly quota), then the narrower
  // audit_log rate-limit-window query — then buildMemoryHint's own
  // ai_scoped_memory + ai_general_memory lookups (run in parallel via
  // Promise.all — ai_scoped_memory's own query is issued first since
  // it's first in that array), the Identity Context block's own
  // college-name lookup (Phase 3 Group (c)), then getAiConfig's own
  // college_ai_config lookup — no tool ever ran, so no Business Service
  // call and no audit row either.
  assert.equal(client.queries.length, 7);
  assert.match(client.queries[0].text, /FROM configurations/);
  assert.match(client.queries[1].text, /FROM ai_usage_counters/);
  assert.match(client.queries[2].text, /FROM audit_log/);
  assert.match(client.queries[3].text, /FROM ai_scoped_memory/);
  assert.match(client.queries[4].text, /FROM ai_general_memory/);
  assert.match(client.queries[5].text, /FROM colleges/);
  assert.match(client.queries[6].text, /FROM college_ai_config/);
});
...
test('aiService.askAgent: the LLM picks an unknown/hallucinated tool name -> a clean AiToolNotFoundError, not a crash', async () => {
  ...
  // have an opinion about at all. Nine queries ran: askAgent's own
  // usage-limits preflight (3 — see the "unconfigured LLM provider" test
  // above for the full breakdown, ARCNAVE modernization P2 / PDF D4's
  // ai_usage_counters split), buildMemoryHint's own ai_scoped_memory +
  // ai_general_memory lookups, the Identity Context block's own
  // college-name lookup (Phase 3 Group (c)), getAiConfig's own
  // college_ai_config lookup — all before the LLM call — then one
  ...
  // resolved to a name the registry then rejected) and (D4) one
  // ai_usage_counters increment alongside it.
...
test("aiService.askAgent: the LLM picks no tool -> returns its direct answer, still wrapped in the Prompt Safety Layer's envelope", async () => {
  ...
  // No tool ran — no Business Service call, no ai_tool_invoked row. Nine
  // queries ran: askAgent's own usage-limits preflight (3 — see the
  // "unconfigured LLM provider" test above for the full breakdown,
  // ARCNAVE modernization P2 / PDF D4's ai_usage_counters split),
  ...
  // Group (c)), getAiConfig's own college_ai_config lookup, then one
  ...
  // block came back and no tool was picked) and (D4) one ai_usage_counters
  // increment alongside it.
  assert.equal(client.queries.length, 9);
  assert.match(client.queries[0].text, /FROM configurations/);
  assert.match(client.queries[1].text, /FROM ai_usage_counters/);
  assert.match(client.queries[2].text, /FROM audit_log/);
