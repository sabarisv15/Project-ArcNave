...
test('discoverRelevantTools: valid tool-name selection is returned with viaToolSearch true', async () => {
  ...
  await withStub(
    ...
    'getToolSearchConfig',
    () => enabledConfig(async () => toolCallDecision(['tool_0', 'tool_2'])),
    async () => {
      ...
      withStub(
        ...
        () =>
          enabledConfig(async () => ({
            type: 'tool_call',
            toolName: aiToolSearchService.SELECT_TOOLS_META_TOOL_NAME,
            arguments: { names: 'not json at all' },
          })),
        async () => {
          const result = await aiToolSearchService.discoverRelevantTools({}, { roleTools: tools, question: 'q' });
...
test('discoverRelevantTools: a name with stray leading/trailing whitespace (real quirk measured against qwen/qwen3-next-80b-a3b-thinking-maas) still matches, not rejected as invalid', async () => {
  ...
  await withStub(
    configurationService,
    'getToolSearchConfig',
    () => enabledConfig(async () => toolCallDecision([' tool_0', 'tool_1 '])),
    async () => {
      const result = await aiToolSearchService.discoverRelevantTools({}, { roleTools: tools, question: 'q' });
      assert.deepEqual(
        result.tools.map((t) => t.name),
...
test('discoverRelevantTools: a name outside roleTools is rejected, never trusted blindly', async () => {
  ...
  await withStub(
    configurationService,
    'getToolSearchConfig',
    () => enabledConfig(async () => toolCallDecision(['tool_0', 'fake_admin_delete_everything'])),
    async () => {
      const result = await aiToolSearchService.discoverRelevantTools({}, { roleTools: tools, question: 'q' });
      assert.deepEqual(
        result.tools.map((t) => t.name),
...
test('discoverRelevantTools: duplicate names are deduped', async () => {
  ...
  await withStub(
    configurationService,
    'getToolSearchConfig',
    () => enabledConfig(async () => toolCallDecision(['tool_0', 'tool_0', 'tool_1'])),
    async () => {
      const result = await aiToolSearchService.discoverRelevantTools({}, { roleTools: tools, question: 'q' });
      assert.deepEqual(
        result.tools.map((t) => t.name),
...
test('discoverRelevantTools: dynamic tool count — a simple question returns 1, a compound one returns more, no fixed K', async () => {
  ...
  await withStub(
    configurationService,
    'getToolSearchConfig',
    () => enabledConfig(async () => toolCallDecision(['tool_3'])),
    async () => {
  ...
  await withStub(
    configurationService,
    'getToolSearchConfig',
    () => enabledConfig(async () => toolCallDecision(['tool_1', 'tool_2', 'tool_3', 'tool_4', 'tool_5'])),
    async () => {
...
test('discoverRelevantTools: a pathological response is capped at MAX_TOOL_SEARCH_RESULTS, never unbounded', async () => {
  ...
  await withStub(
    configurationService,
    'getToolSearchConfig',
    () => enabledConfig(async () => toolCallDecision(tools.map((t) => t.name))),
    async () => {
      const result = await aiToolSearchService.discoverRelevantTools({}, { roleTools: tools, question: 'q' });
  ...
  await withStub(
    ...
    () =>
      withStub(
        ...
        () =>
          enabledConfig(async () => {
            throw new Error('no Vertex AI MaaS provider is configured');
          }),
        async () => {
          const result = await aiToolSearchService.discoverRelevantTools({}, { roleTools: tools, question: 'q' });
          assert.equal(result.viaToolSearch, false);
...
test('discoverRelevantTools: no identityContext/credentials are ever passed into the Tool Search call', async () => {
  ...
  await withStub(
    ...
    'getToolSearchConfig',
    () =>
      enabledConfig(async (cfg, context) => {
  ...
  assert.ok(
    !/identityContext|apiKey|password|token|collegeId/i.test(serialized),
    'no identity/credential data reached the Tool Search context',
  );
});
    ...
    () =>
      withStub(
        ...
        async () => {
          ...
          assert.equal(result.completed, true);
          assert.equal(
            result.viaToolSearch,
            false,
            'the response itself is still untrusted for tool selection purposes',
          );
        },
      ),
  );
      ...
      withStub(
        ...
        () => {
          ...
          return {
            provider: 'vertex_maas',
            config: { model },
            adapter: { completeWithTools: async () => ({ type: 'answer', text: 'no tool call' }) },
          };
        },
        async () => {
          const result = await aiToolSearchService.discoverRelevantTools({}, { roleTools: tools, question: 'q' });
          assert.equal(
            resolveCount,
            1,
            'getToolSearchConfig must be resolved exactly once per request, even on the distrusted-response fallback path',
          );
      ...
      withStub(
        ...
        async () => {
          const result = await aiToolSearchService.discoverRelevantTools(
            {},
            { roleTools: tools, question: 'attendance only' },
          );
          assert.deepEqual(
            result.tools.map((t) => t.name),
            ['tool_0', 'tool_1'],
  ...
  await withStub(
    ...
    () =>
      withStub(
        ...
        () =>
          enabledConfig(async () =>
            toolCallDecision(['attendance_summary', 'student_profile'], {
        ...
        async () => {
          ...
          assert.deepEqual(
            result.tools.map((t) => t.name),
            ['attendance_summary', 'student_profile'],
          );
          assert.equal(
            result.viaToolSearch,
            true,
            'a valid subset, even if incomplete, is not treated as an outright Tool Search failure',
          );
      ...
      withStub(
        ...
        async () => {
          ...
          assert.equal(
            ...
            'complete',
            'coverage becomes complete once the broader path actually finds the missing capability',
          );
          assert.deepEqual(result.uncoveredRequirements, []);
        },
      ),
      ...
      withStub(
        ...
        async () => {
          ...
          assert.equal(
            result.coverageStatus,
            'uncertain',
            'the broader path re-confirmed the same tool_0 with nothing new — the original uncertainty stands',
          );
          assert.deepEqual(
            result.tools.map((t) => t.name),
            ['tool_0'],
  ...
  await withStub(
    ...
    () =>
      withStub(
        ...
        () =>
          enabledConfig(async () =>
            toolCallDecision(['hallucinated_tool_a', 'hallucinated_tool_b'], { coverageStatus: 'complete' }),
          ),
        async () => {
          const result = await aiToolSearchService.discoverRelevantTools({}, { roleTools: tools, question: 'q' });
          assert.equal(
            result.viaToolSearch,
            false,
            'all-invalid names must still fall back regardless of the reported coverageStatus',
          );
          assert.deepEqual(
            result.tools.map((t) => t.name),
            ['tool_0', 'tool_1'],
      ...
      withStub(
        ...
        async () => {
          ...
          assert.equal(
            result.coverageStatus,
            'uncertain',
            'a missing coverageStatus must never be treated as an implicit "complete"',
          );
          assert.deepEqual(
            result.tools.map((t) => t.name),
            ['tool_0'],
      ...
      withStub(
        ...
        async () => {
          ...
          assert.deepEqual(
            ...
            [],
            'a non-array uncoveredRequirements must be normalized away, not passed through',
          );
        },
      ),
  );
    ...
    () =>
      withStub(
        ...
        async () => {
          ...
          assert.ok(
            result.uncoveredRequirements[0].length <= 200,
            'each uncoveredRequirements item must be bounded in length',
          );
        },
      ),
  );
