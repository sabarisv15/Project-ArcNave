...
test('markAttendance raises an absence flag on the 6th consecutive full-day absence (RS-ATT-008)', async (t) => {
  ...
  await t.test(
    'raises exactly one flag on the 6th consecutive full-day absence, and skips if one is already outstanding',
    async () => {
      const getClassMock = t.mock.method(academicService, 'getClass', async () => APPROVED_CLASS);
      const resolveTutorMock = t.mock.method(identityService, 'resolvePositionOccupant', async () => 'tutor-user');
      const findMock = t.mock.method(attendanceRepository, 'findByClassSessionAndHour', async () => null);
      ...
      const findOutstandingMock = t.mock.method(
        attendanceAbsenceFlagRepository,
        'findOutstandingForStudents',
        async () => [],
      );
      const createFlagMock = t.mock.method(attendanceAbsenceFlagRepository, 'create', async (client, fields) => ({
        id: 'flag-1',
        ...fields,
      }));
      ...
      assert.equal(
        auditMock.mock.calls.some((c) => c.arguments[1].action === 'attendance_absence_flag_raised'),
        true,
      );
  ...
  await t.test(
    'a concurrent double-raise (23505 on the partial unique index) is swallowed quietly, not thrown',
    async () => {
      const getClassMock = t.mock.method(academicService, 'getClass', async () => APPROVED_CLASS);
      const resolveTutorMock = t.mock.method(identityService, 'resolvePositionOccupant', async () => 'tutor-user');
      const findMock = t.mock.method(attendanceRepository, 'findByClassSessionAndHour', async () => null);
      ...
      const createFlagMock = t.mock.method(attendanceAbsenceFlagRepository, 'create', async () => {
        const err = new Error(
          'duplicate key value violates unique constraint "attendance_absence_flags_student_outstanding_key"',
        );
        err.code = '23505';
  ...
  await t.test('does not raise a second flag while one is already outstanding for the student', async () => {
    ...
    const findOutstandingMock = t.mock.method(
      attendanceAbsenceFlagRepository,
      'findOutstandingForStudents',
      async () => [{ id: 'flag-existing', student_id: 'stu-1' }],
    );
    const createFlagMock = t.mock.method(attendanceAbsenceFlagRepository, 'create');
    t.after(() => {
      getClassMock.mock.restore();
  ...
  await t.test(
    'multiple newly-absent students share one session-history query and one batched outstanding-flag check, not one each',
    async () => {
      const getClassMock = t.mock.method(academicService, 'getClass', async () => APPROVED_CLASS);
      const resolveTutorMock = t.mock.method(identityService, 'resolvePositionOccupant', async () => 'tutor-user');
      const findMock = t.mock.method(attendanceRepository, 'findByClassSessionAndHour', async () => null);
      ...
      await attendanceService.markAttendance(
        {},
        {
          ...
          hourIndex: 1,
          absentStudentIds: ['stu-1', 'stu-2', 'stu-3'],
          totalStudents: 40,
        },
        { actorUserId: 'tutor-user', actorRole: 'class_tutor' },
      );
...
test('closeAbsenceFlag', async (t) => {
  ...
  await t.test("the flag's own department hod may close it", async () => {
    ...
    const closeMock = t.mock.method(attendanceAbsenceFlagRepository, 'close', async (client, id, fields) => ({
      id,
    ...
    const result = await attendanceService.closeAbsenceFlag({}, 'flag-1', {
      actorUserId: 'hod-1',
      actorRole: 'hod',
      remarks: 'contacted parent',
    });
...
test('listOutstandingAbsenceFlagsForActor', async (t) => {
  await t.test("passes the actor's visible classIds straight through", async () => {
    ...
    const result = await attendanceService.listOutstandingAbsenceFlagsForActor(
      {},
      { actorUserId: 'hod-1', actorRole: 'hod', collegeId: 'c1' },
    );
  ...
  await t.test('an unrestricted actor (principal, null classIds) is never filtered', async () => {
    ...
    await attendanceService.listOutstandingAbsenceFlagsForActor(
      {},
      { actorUserId: 'principal-1', actorRole: 'principal', collegeId: 'c1' },
    );
  });
});
