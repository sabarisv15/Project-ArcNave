...
function mockHappyPath(t, { createResult, updateResult } = {}) {
  ...
  // Stage 8a / RS-GOV-013: uploadDocument/downloadDocument/etc. now
  // resolve the college's storage provider via configurationService
  // before touching fileStorage — null (never configured) is the real
  // "no row yet" shape getConfiguration returns, resolving to the
  // registry's own default provider, same as every real caller sees.
  const getConfigurationMock = t.mock.method(configurationService, 'getConfiguration', async () => null);
  // Storage-quota enforcement (assertWithinStorageQuota) — null tier
  // means unmetered, same as every college onboarded before this
  // existed; matches the real default for a fake 'c1' test college.
  const getStorageTierMock = t.mock.method(collegeProfileRepository, 'getStorageTier', async () => null);
  const createMock = t.mock.method(
    documentRepository,
    'create',
    async (client, fields) => createResult || { id: 'doc-1', college_id: fields.collegeId, status: 'uploaded' },
  );
  ...
  const softDeleteMock = t.mock.method(documentRepository, 'softDelete', async (client, id) => ({
    id,
    college_id: 'c1',
    deleted_at: new Date(),
  }));
  ...
  t.after(() => {
    ...
    getConfigurationMock.mock.restore();
    getStorageTierMock.mock.restore();
...
test('DocumentService validation, actor stamping, and audit logging (no DB, no filesystem)', async (t) => {
  ...
  await t.test(
    'uploadDocument never accepts a caller-supplied status — always writes through the repository default',
    async () => {
  ...
  await t.test(
    'uploadDocument maps a documents_student_id_fkey violation to DocumentStudentNotFoundError',
    async () => {
      mockHappyPath(t);
  ...
  await t.test(
    "reviewDocument accepts verified and rejected, stamping the actor and a timestamp, never the caller's",
    async () => {
      ...
      await documentService.reviewDocument(
        {},
        'doc-1',
        { status: 'verified', remarks: 'looks good' },
        { actorUserId: 'u2' },
      );
      ...
      await documentService.reviewDocument(
        {},
        'doc-2',
        { status: 'rejected', remarks: 'blurry' },
        { actorUserId: 'u3' },
      );
  ...
  await t.test('removeDocument soft-deletes without touching fileStorage at all', async () => {
    ...
    const readFileMock = t.mock.method(fileStorage, 'readFile', async () => {
  ...
  await t.test(
    "downloadDocument returns null without calling fileStorage.readFile when the row doesn't exist",
    async () => {
  ...
  await t.test("downloadDocument reads bytes from storage using the row's storage_path", async () => {
    const findByIdMock = t.mock.method(documentRepository, 'findById', async () => ({
      id: 'doc-1',
      college_id: 'c1',
      storage_path: 'c1/s1/aadhaar/x.pdf',
    }));
    const readFileMock = t.mock.method(fileStorage, 'readFile', async (relativePath) =>
      Buffer.from(`bytes-for-${relativePath}`),
    );
    const getConfigurationMock = t.mock.method(configurationService, 'getConfiguration', async () => null);
    t.after(() => {
      findByIdMock.mock.restore();
  ...
  await t.test(
    'uploadTemplate rejects a non-.docx buffer at upload time, without touching storage or the DB',
    async () => {
      const { buildPathMock, createMock } = mockHappyPath(t);
        ...
        () =>
          documentService.uploadTemplate(
            {},
            {
              ...
              fileBuffer: Buffer.from('plain text, not a zip'),
            },
            { actorUserId: 'u1' },
          ),
  ...
  await t.test('uploadTemplate accepts a real .docx (valid zip with word/document.xml)', async () => {
    ...
    await documentService.uploadTemplate(
      {},
      {
        ...
        fileName: 'real.docx',
        mimeType: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document',
        fileBuffer: buildFakeDocxBuffer(),
      },
      { actorUserId: 'u1' },
    );
  ...
  await t.test(
    'uploadInstitutionalDocument rejects a categoryId that does not resolve to a real category',
    async () => {
      const { createMock } = mockHappyPath(t);
  ...
  function mockNoDuplicates(t) {
    ...
    t.after(() => {
      ...
      similarMock.mock.restore();
    });
    return { hashMock, similarMock };
  }

  await t.test(
    'uploadInstitutionalDocument resolves category/active-year, forces studentId to null, and derives docType from the category slug',
    async () => {
      const { createMock, updateMock } = mockHappyPath(t);
      mockNoDuplicates(t);
      const getCategoryMock = t.mock.method(documentCategoryService, 'getCategory', async () => ({
        id: 'cat-1',
        slug: 'circular',
        name: 'Circulars',
      }));
      const getActiveYearMock = t.mock.method(academicYearService, 'getActiveAcademicYear', async () => ({
        id: 'year-1',
      }));
      t.after(() => {
        getCategoryMock.mock.restore();
        getActiveYearMock.mock.restore();
      });
  ...
  await t.test(
    'uploadInstitutionalDocument blocks an exact-duplicate (content_hash match) unless confirmUpload is set',
    async () => {
      mockHappyPath(t);
      const getCategoryMock = t.mock.method(documentCategoryService, 'getCategory', async () => ({
        id: 'cat-1',
  ...
  await t.test(
    'uploadInstitutionalDocument with a documentGroupId uploads a new version, skipping duplicate detection',
    async () => {
      const { createMock, updateMock } = mockHappyPath(t);
      const hashMock = t.mock.method(documentRepository, 'findByContentHash', async () => {
        throw new Error('must not be called when versioning');
      });
  ...
  await t.test(
    'listInstitutionalDocuments delegates to documentRepository.findInstitutional with the given filters, and a staff-tier role applies no publication_status filter',
    async () => {
  ...
  await t.test('mergeDocumentTemplate persists the merged bytes as a new document via uploadDocument', async () => {
    ...
    const findByIdMock = t.mock.method(documentRepository, 'findById', async () => ({
      id: 'tmpl-1',
      doc_type: documentService.TEMPLATE_DOC_TYPE,
      college_id: 'c1',
      file_name: 'cert.docx',
      storage_path: 'c1/templates/cert.docx',
    }));
...
test('DocumentService.assertCanViewDocument (no DB)', async (t) => {
  ...
  await t.test(
    "a student-linked document rejects a caller outside the student's scope (staff from a different class)",
    async () => {
      const viewMock = t.mock.method(visibilityService, 'assertCanViewStudent', async () => {
  ...
  await t.test(
    'a generated report (student_id null, not a template) is readable by the user who generated it',
    async () => {
      await documentService.assertCanViewDocument(
        {},
        {
  ...
  await t.test(
    'a generated report (student_id null, not a template) is rejected for an unrelated staff member — not broadly readable',
    async () => {
      await assert.rejects(
        () =>
          documentService.assertCanViewDocument(
  ...
  await t.test(
    'an institutional document (category_id set) is readable by staff-tier roles regardless of publication_status',
    async () => {
      await documentService.assertCanViewDocument(
        {},
        {
...
test('DocumentService Phase 3 — versions, lineage, publish/supersede/archive (no DB)', async (t) => {
  ...
  await t.test(
    'compareDocumentVersions surfaces metadata differences and flags identical content via content_hash',
    async () => {
  ...
  await t.test(
    'linkDocumentLineage sets lineage_parent_id after validating both documents exist and rejects a direct self-link',
    async () => {
      const findByIdMock = t.mock.method(documentRepository, 'findById', async (client, id) => ({
        id,
        college_id: 'c1',
      ...
      await assert.rejects(
        () =>
          documentService.linkDocumentLineage(
            {},
            { documentId: 'doc-x', previousYearDocumentId: 'doc-x' },
            { actorUserId: 'u1' },
          ),
  ...
  await t.test('linkDocumentLineage rejects a cycle (A -> B -> A)', async () => {
    ...
    await assert.rejects(
      () =>
        documentService.linkDocumentLineage(
          {},
          { documentId: 'doc-b', previousYearDocumentId: 'doc-a' },
          { actorUserId: 'u1' },
        ),
      documentService.DocumentLineageError,
    );
  ...
  await t.test('getDocumentLineage returns ancestors oldest-first and descendants', async () => {
    const rows = {
      y2026: {
        ...
        lineage_parent_id: 'y2025',
      },
      y2025: {
        id: 'y2025',
        title: '2025 Curriculum',
        lineage_parent_id: 'y2024',
      },
      y2024: { id: 'y2024', title: '2024 Curriculum', lineage_parent_id: null },
    };
    const findByIdMock = t.mock.method(documentRepository, 'findById', async (client, id) => rows[id] || null);
    const findByLineageParentIdMock = t.mock.method(documentRepository, 'findByLineageParentId', async () => [
      { id: 'y2027', title: '2027 Curriculum' },
    ]);
    t.after(() => {
      findByIdMock.mock.restore();
      findByLineageParentIdMock.mock.restore();
    });

    const lineage = await documentService.getDocumentLineage({}, 'y2026');
    assert.deepEqual(
      lineage.ancestors.map((a) => a.id),
      ['y2024', 'y2025'],
    );
    assert.deepEqual(
      lineage.descendants.map((d) => d.id),
      ['y2027'],
    );
  });

  await t.test(
    'submitPublishRequest only accepts a Draft document and routes through workflowService with a principal-only chain',
    async () => {
      const findByIdMock = t.mock.method(documentRepository, 'findById', async () => ({
        id: 'doc-1',
        college_id: 'c1',
  ...
  await t.test(
    'approvePublish sets Published and automatically supersedes a previously Published sibling in the same group',
    async () => {
      const rows = {
        'doc-2': {
          id: 'doc-2',
      ...
      t.after(() => {
        ...
        findByGroupIdMock.mock.restore();
        updateMock.mock.restore();
        auditMock.mock.restore();
      });
      ...
      assert.equal(
        updateMock.mock.callCount(),
        2,
        'must update both the superseded sibling and the newly published document',
      );
  ...
  await t.test(
    'archiveInstitutionalDocument is a direct action (no workflowService call) and only accepts Published/Superseded',
    async () => {
      const findByIdMock = t.mock.method(documentRepository, 'findById', async () => ({
        id: 'doc-1',
        college_id: 'c1',
      ...
      const submitMock = t.mock.method(workflowService, 'submitRequest', async () => {
...
test('DocumentService personal-document rename/move/duplicate (no DB, no filesystem)', async (t) => {
  await t.test('renamePersonalDocument returns null for an unknown id', async () => {
    ...
    const result = await documentService.renamePersonalDocument(
      {},
      'missing',
      { fileName: 'x.pdf' },
      { actorUserId: 'u1' },
  ...
  await t.test('renamePersonalDocument updates file_name (and title when given) for the real owner', async () => {
    ...
    const result = await documentService.renamePersonalDocument(
      {},
      'doc-1',
      { fileName: '  Notes v2.pdf  ', title: 'Notes v2' },
      { actorUserId: 'u1' },
    );
  ...
  await t.test('movePersonalDocument rejects a folder name the actor does not own', async () => {
    ...
    const listMock = t.mock.method(personalDocumentFolderRepository, 'listByOwner', async () => [
      { id: 'f1', name: 'Research' },
    ]);
    t.after(() => {
      findByIdMock.mock.restore();
