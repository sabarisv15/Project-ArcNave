...
async function insertDepartment(pool, collegeId, name) {
  const result = await pool.query('INSERT INTO departments (college_id, name) VALUES ($1, $2) RETURNING id', [
    collegeId,
    name,
  ]);
  return result.rows[0].id;
...
async function insertClass(pool, collegeId, className) {
  const result = await pool.query('INSERT INTO classes (college_id, class_name) VALUES ($1, $2) RETURNING id', [
    collegeId,
    className,
  ]);
  return result.rows[0].id;
...
async function makePosition(pool, { collegeId, level, title, createdBy }) {
  ...
  const account = await positionRepository.createPositionAccount(pool, {
    ...
    officialEmail: `${title}@example.edu`,
    passwordHash: 'x',
  });
...
test('identity resolvers (Phase 3)', async (t) => {
  ...
  await t.test(
    'positionResolver: resolves the position a user actively occupies, and stops after revocation',
    async () => {
  ...
  await t.test(
    'visibilityResolver: Level 1 -> college scope, Level 3 -> department scope, no position -> self_assigned',
    async () => {
      ...
      const principalPositions = await positionResolver.resolveActivePositions(pool, {
        collegeId,
      ...
      await positionRepository.createPositionDepartmentAssignment(pool, {
        collegeId,
        positionId: hodPosition.position.id,
        departmentId: deptId,
        assignedBy: hodUserId,
      });
  ...
  await t.test(
    'visibilityResolver: Level 2 falls back to self_assigned with no configured policy, department scope once Level 1 assigns one',
    async () => {
      ...
      await positionRepository.createPositionDepartmentAssignment(pool, {
        collegeId,
        positionId: level2Position.position.id,
        departmentId: deptId,
        assignedBy: level2UserId,
      });
  ...
  await t.test(
    'identityService.resolveCapabilities: composes all five resolvers into one comparable structure',
    async () => {
  ...
  await t.test(
    'identityService.resolveCapabilities: a user with no active position resolves as staff/self_assigned, not an error',
    async () => {
  ...
  await t.test(
    'identityService.resolvePositionOccupant: resolves a college-level slot and a department slot to their real occupants',
    async () => {
  ...
  await t.test(
    'identityService.resolvePositionOccupant: returns null for a vacant/unprovisioned slot, never throws',
    async () => {
  ...
  await t.test(
    'identityService.resolveCapabilitiesForPosition: Level 1 -> principal/college, own moduleKeys only',
    async () => {
      ...
      const capabilities = await identityService.resolveCapabilitiesForPosition(pool, {
  ...
  await t.test(
    'identityService.resolveCapabilitiesForPosition: Level 4 + position_type=class_tutor -> class_tutor/class, classIds now real (classResolver, Phase 2 step 8)',
    async () => {
  ...
  await t.test(
    'identityService.resolveCapabilitiesForPosition: throws PositionAccountNotFoundError for an unknown positionAccountId',
    async () => {
  ...
  await t.test(
    "identityService.resolveCapabilitiesForPosition: one occupant holding TWO positions gets each position's OWN scope, never unioned",
    async () => {
      ...
      await positionRepository.createPositionModuleAssignment(pool, {
        collegeId,
        positionId: principal.position.id,
        moduleKey: 'academics',
        assignedBy: occupantUserId,
      });
  ...
  await t.test(
    'identityService.resolvePositionOccupant: resolves a class slot to its active Class Tutor, null once vacant',
    async () => {
  ...
  await t.test(
    "identityService.resolveActiveClassTutorPosition: resolves a user's active Class Tutor seat to its mapped class, null otherwise",
    async () => {
      ...
      const beforeAssignment = await identityService.resolveActiveClassTutorPosition(pool, {
        userId: tutorUserId,
        collegeId,
      });
      ...
      const afterAssignment = await identityService.resolveActiveClassTutorPosition(pool, {
        userId: tutorUserId,
        collegeId,
      });
      assert.equal(afterAssignment, classId);

      const plainStaffResult = await identityService.resolveActiveClassTutorPosition(pool, {
        userId: plainStaffUserId,
        collegeId,
      });
  ...
  // 4-login authorization architecture (2026-08-09): superseded the
  // Phase 2 step 12 version of this test, which asserted the OPPOSITE
  // — that a personal login's assignedClassIds included the tutor-of-
  // record class. That merge is exactly what this architecture
  // removes: Position Occupancy ("A" — this person currently occupies
  // the L4 seat) must never widen a personal Staff login's
  // (resolveCapabilities') scope, only the L4 Position Account login
  // itself (resolveCapabilitiesForPosition, covered above) may carry
  // class_tutor scope. resolveActiveClassTutorPosition remains correct
  // and unchanged — it's the function's use inside resolveCapabilities
  // that was removed, not the function itself (see identityService.
  // getOwnProfile's own is_class_tutor display use, elsewhere).
  await t.test(
    'identityService.resolveCapabilities: assignedClassIds does NOT include an active Class Tutor position (personal login stays Staff-scoped only)',
    async () => {
      ...
      // Position Occupancy is still correctly resolvable (informational)...
      const tutorClassId = await identityService.resolveActiveClassTutorPosition(pool, {
        userId: tutorUserId,
        collegeId,
      });
      assert.equal(tutorClassId, classId);

      // ...but this same person's PERSONAL login gets none of it.
      const capabilities = await identityService.resolveCapabilities(pool, { userId: tutorUserId, collegeId });
      assert.equal(capabilities.scopeLevel, SCOPE_LEVELS.SELF_ASSIGNED);
      assert.deepEqual(capabilities.assignedClassIds, []);
    },
  );
});
