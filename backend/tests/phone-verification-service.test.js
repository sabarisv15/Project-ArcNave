...
test('phoneVerificationService.requestOtp', async (t) => {
  ...
  await t.test(
    "rejects a caller outside the student's tutor/hod/principal scope, before creating any OTP row",
    async () => {
      const findMock = t.mock.method(studentRepository, 'findById', async () => ({
        id: 'student-1',
        college_id: 'c1',
      ...
      const createMock = t.mock.method(studentPhoneOtpRepository, 'create', async () => {
  ...
  await t.test('creates an OTP row, sends via whatsapp (never sms/email), and audit-logs', async () => {
    ...
    const createMock = t.mock.method(studentPhoneOtpRepository, 'create', async (client, fields) => ({
      id: 'otp-1',
      ...fields,
    }));
...
test('phoneVerificationService.verifyOtp', async (t) => {
  ...
  await t.test(
    "rejects a caller outside the student's tutor/hod/principal scope, before checking any OTP row",
    async () => {
      const findStudentMock = t.mock.method(studentRepository, 'findById', async () => ({
        id: 'student-1',
        college_id: 'c1',
  ...
  await t.test('marks the OTP consumed and sets phone_verified on a correct code for target=phone', async () => {
    ...
    const student = await phoneVerificationService.verifyOtp({}, 'student-1', 'phone', '123456', {
      actorUserId: 'user-1',
    });
