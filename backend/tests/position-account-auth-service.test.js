...
test('positionAccountAuthService.login (no DB)', async (t) => {
  ...
  await t.test('mandatory institution MFA gates a Position Account login into a challenge, not a token pair', async () => {
    const passwordHash = await security.hashPassword('correct-horse');
    const findMock = t.mock.method(positionRepository, 'findPositionAccountByOfficialEmail', async () => ({
      id: 'acct-1', position_id: 'pos-1', college_id: 'c1', password_hash: passwordHash, token_version: 0, official_email: 'hod@example.edu', mfa_enabled: false,
    }));
    const positionMock = t.mock.method(positionRepository, 'findPositionById', async () => ({
      id: 'pos-1', level: 3, position_type: null,
    }));
    const authConfigMock = t.mock.method(authService, 'getAuthConfig', async () => ({ mfaMode: 'mandatory', mfaRoles: null }));
    const otpCreateMock = t.mock.method(positionAccountMfaOtpRepository, 'create', async () => ({ id: 'challenge-1' }));
    const emailMock = t.mock.method(notificationService, 'sendMfaCodeEmail', async () => ({ status: 'sent' }));
    const createTokenMock = t.mock.method(positionRepository, 'createPositionAccountRefreshToken', async () => { throw new Error('must not issue a token pair while MFA is pending'); });
    mockAudit(t);
    t.after(() => {
      findMock.mock.restore();
      positionMock.mock.restore();
  ...
  await t.test('mfaRoles scoping: a role not in scope skips MFA even under mandatory mode', async () => {
    ...
    const authConfigMock = t.mock.method(authService, 'getAuthConfig', async () => ({ mfaMode: 'mandatory', mfaRoles: ['principal'] }));
    const createTokenMock = t.mock.method(positionRepository, 'createPositionAccountRefreshToken', async () => {});
    mockAudit(t);
    t.after(() => {
      findMock.mock.restore();
...
test('positionAccountAuthService.verifyPositionMfaLogin (no DB)', async (t) => {
  function mockLiveChallenge(t, overrides = {}) {
    const challenge = {
      id: 'challenge-1', position_account_id: 'acct-1', code_hash: hashCode('123456'), attempts: 0, consumed_at: null, expires_at: new Date(Date.now() + 60000), ...overrides,
    };
    ...
    t.after(() => { findMock.mock.restore(); consumeMock.mock.restore(); });
  }
  ...
  await t.test('re-checks position eligibility at verify time, not just at challenge-issue time', async () => {
    ...
    const findAccountMock = t.mock.method(positionRepository, 'findPositionAccountById', async () => (
      { id: 'acct-1', college_id: 'c1', position_id: 'pos-1', token_version: 0 }
    ));
    ...
    mockAudit(t);
    t.after(() => { findAccountMock.mock.restore(); findPositionMock.mock.restore(); });
  ...
  await t.test('issues a token pair when the position is still eligible', async () => {
    ...
    const createTokenMock = t.mock.method(positionRepository, 'createPositionAccountRefreshToken', async () => {});
    mockAudit(t);
    t.after(() => { findAccountMock.mock.restore(); findPositionMock.mock.restore(); createTokenMock.mock.restore(); });

    const result = await positionAccountAuthService.verifyPositionMfaLogin({}, { challengeId: 'challenge-1', code: '123456' });
    assert.equal(typeof result.accessToken, 'string');
  });
});
...
test('positionAccountAuthService MFA enrollment (no DB)', async (t) => {
  await t.test('enablePositionMfa/disablePositionMfa flip the flag and audit the action', async () => {
    const setMock = t.mock.method(positionRepository, 'setPositionAccountMfaEnabled', async (client, id, enabled) => (
      { id, college_id: 'c1', position_id: 'pos-1', mfa_enabled: enabled }
    ));
    const auditMock = mockAudit(t);
