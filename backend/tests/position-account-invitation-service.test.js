...
test('assertCanInvite: Level 3 (HOD) requires a Level 1 tenant actor in the same college (RS-IDN-004 — L2 is optional, no L3 action may require one)', async (t) => {
  ...
  await t.test('regression (D2/ADL-006): an L1 at a college with NO L2 present may still invite an L3 — actorCapabilities carries only the Level 1 position, no Level 2 entry anywhere', () => {
    assert.doesNotThrow(() => assertCanInvite({
      actorIsPlatformAdmin: false,
      actorCapabilities: { collegeId: 'c1', positions: [{ level: 1 }] },
      targetLevel: 3,
