...
const ACCOUNT = {
  ...
  college_id: 'c1',
  position_id: 'pos-1',
  official_email: 'hod-cse@positions.internal',
  token_version: 0,
};
const POSITION = {
  id: 'pos-1',
...
function mockCommon(t, { currentOccupant = null } = {}) {
  ...
  const revokeAllRefreshMock = t.mock.method(
    positionRepository,
  ...
  const createInvitationMock = t.mock.method(positionAccountInvitationRepository, 'createInvitation', async () => ({
    id: 'inv-1',
    expires_at: new Date(),
  }));
...
test('reassignPositionOccupant', async (t) => {
  await t.test('rejects a missing newOccupantUserId, without touching the DB', async () => {
    ...
    await assert.rejects(
      () =>
        positionAccountInvitationService.reassignPositionOccupant(
          {},
          { positionAccountId: 'acct-1', actorUserId: 'actor-1' },
        ),
  ...
  await t.test('throws PositionAccountReassignmentNotFoundError for an unknown positionAccountId', async () => {
    ...
    await assert.rejects(
      () =>
        positionAccountInvitationService.reassignPositionOccupant(
          {},
          { positionAccountId: 'missing', newOccupantUserId: 'u2', actorUserId: 'actor-1' },
        ),
  ...
  await t.test('is idempotent — reassigning to the SAME occupant is a no-op, no session/credential reset', async () => {
    ...
    const result = await positionAccountInvitationService.reassignPositionOccupant(
      {},
      {
        positionAccountId: 'acct-1',
        newOccupantUserId: 'u1',
        actorUserId: 'actor-1',
      },
    );
  ...
  await t.test(
    'reassigning to a DIFFERENT occupant revokes the old one, resets sessions/credentials, and links the new occupant',
    async () => {
      const mocks = mockCommon(t, { currentOccupant: { id: 'occ-1', user_id: 'u1' } });
  ...
  await t.test(
    'filling a previously VACANT seat still runs the full lifecycle (fresh invite, no prior occupant to revoke)',
    async () => {
