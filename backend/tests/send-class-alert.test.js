...
test('academicService.sendClassAlert', async (t) => {
  ...
  await t.test(
    'throws ClassSendAlertNotAssignedError when the actor is neither tutor nor assigned faculty',
    async () => {
  ...
  await t.test(
    "sends to each student's verified phone and verified parent_phone, skips unverified numbers, and audit-logs a summary",
    async () => {
      ...
      const sendMock = t.mock.method(notificationService, 'sendViaChannel', async (client, args) => ({
        ...
        status: args.to === '+15552222222' ? 'failed' : 'sent',
        to: args.to,
        error: args.to === '+15552222222' ? 'not opted in' : undefined,
      }));
      const auditMock = t.mock.method(auditLogRepository, 'createAuditLogEntry', async () => {});
      t.after(() => {
      ...
      const results = await academicService.sendClassAlert({}, 'class-1', 'Tomorrow is a holiday', {
        actorUserId: 'tutor-1',
  ...
  await t.test(
    'returns an empty result list (still audit-logged) when no student in the class has any verified number',
    async () => {
      const findClassMock = t.mock.method(classRepository, 'findById', async () => ({
        id: 'class-1',
        college_id: 'c1',
      ...
      const sendMock = t.mock.method(notificationService, 'sendViaChannel', async () => {
      ...
      const results = await academicService.sendClassAlert({}, 'class-1', 'hello', {
        actorUserId: 'tutor-1',
        actorRole: 'class_tutor',
      });
  ...
  await t.test(
    "rejects a personal Staff login's tutor-only reach when that person is the real tutor but not timetable-assigned",
    async () => {
      const findMock = t.mock.method(classRepository, 'findById', async () => ({
        id: 'class-1',
        college_id: 'c1',
