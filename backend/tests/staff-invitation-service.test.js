...
test('inviteStaff', async (t) => {
  ...
  await t.test(
    'auto-derives departmentId from the real hod actor, creates the invitation, and sends the email',
    async () => {
      ...
      const createInvitationMock = t.mock.method(
      ...
      const invitation = await staffService.inviteStaff(
        ...
        { actorUserId: 'hod-1', collegeId: 'c1' },
      );
...
test('acceptStaffInvitation', async (t) => {
  ...
  await t.test('rejects a weak password', async () => {
    await assert.rejects(
      () =>
        staffService.acceptStaffInvitation({}, validInvitation, {
          username: 'newstaff',
          password: 'weak',
          fullName: 'New Staff',
        }),
  ...
  await t.test('maps a duplicate username to StaffInvitationUsernameConflictError', async () => {
    ...
    const createUserMock = t.mock.method(authRepository, 'createUser', async () => {
      throw err;
    });
  ...
  await t.test(
    'creates the users+staff rows (inactive, department from the invitation) and auto-submits into the hod->principal chain',
    async () => {
      const createUserMock = t.mock.method(authRepository, 'createUser', async (client, fields) => ({
        id: 'user-1',
        ...fields,
      }));
      const createStaffMock = t.mock.method(staffRepository, 'create', async (client, fields) => ({
        id: 'staff-1',
        college_id: fields.collegeId,
        department_id: fields.departmentId,
        ...fields,
      }));
      const findByIdMock = t.mock.method(staffRepository, 'findById', async () => ({
        id: 'staff-1',
        college_id: 'c1',
        department_id: 'dept-1',
      }));
