...
test('StaffWorkHistoryService (no DB)', async (t) => {
  ...
  await t.test('addOwnWorkHistoryEntry writes the entry against the caller\'s own staff/college id and audits it', async () => {
    const findMock = t.mock.method(staffRepository, 'findByUserId', async () => ({ id: 'staff-1', college_id: 'c1', user_id: 'u1' }));
    ...
    const result = await staffWorkHistoryService.addOwnWorkHistoryEntry({}, {
      institutionName: 'ABC College', designationHeld: 'Lecturer', fromDate: '2020-01-01', toDate: '2022-01-01',
    }, { userId: 'u1' });
  ...
  await t.test('removeOwnWorkHistoryEntry throws StaffWorkHistoryNotFoundError for a nonexistent entry', async () => {
    ...
    t.after(() => { findMock.mock.restore(); findEntryMock.mock.restore(); });
  ...
  await t.test('removeOwnWorkHistoryEntry throws StaffWorkHistoryForbiddenError for another staff member\'s entry', async () => {
    const findMock = t.mock.method(staffRepository, 'findByUserId', async () => ({ id: 'staff-1', college_id: 'c1', user_id: 'u1' }));
    const findEntryMock = t.mock.method(staffWorkHistoryRepository, 'findById', async () => ({ id: 'wh-1', staff_id: 'staff-OTHER' }));
    t.after(() => { findMock.mock.restore(); findEntryMock.mock.restore(); });
  ...
  await t.test('removeOwnWorkHistoryEntry removes an owned entry and audits it', async () => {
    const findMock = t.mock.method(staffRepository, 'findByUserId', async () => ({ id: 'staff-1', college_id: 'c1', user_id: 'u1' }));
    const findEntryMock = t.mock.method(staffWorkHistoryRepository, 'findById', async () => ({ id: 'wh-1', staff_id: 'staff-1' }));
    ...
    const auditMock = t.mock.method(auditLogRepository, 'createAuditLogEntry', async () => {});
    t.after(() => {
      findMock.mock.restore(); findEntryMock.mock.restore(); removeMock.mock.restore(); auditMock.mock.restore();
    });
