...
test('generateSlotGrid', async (t) => {
  ...
  await t.test('rejects an unrecognized working day', async () => {
    await assert.rejects(
      () => academicService.generateSlotGrid({}, 'college-1', {
        workingDays: ['Funday'], startTime: '09:00', endTime: '10:00', slotDurationMinutes: 60,
      }),
      academicService.TimetableConfigValidationError,
    );
  });

  await t.test('builds a slot grid from working days/start/end/duration/break, skipping breaks and existing slots', async () => {
    const existing = new Set(['Monday-1']);
    const findMock = t.mock.method(timetablePeriodRepository, 'findByCollegeDayAndHour', async (client, collegeId, day, hourIndex) => {
      const key = `${day}-${hourIndex}`;
      return existing.has(key) ? { id: key, day_of_week: day, hour_index: hourIndex } : null;
    });
    const createMock = t.mock.method(timetablePeriodRepository, 'create', async (client, fields) => ({ id: `${fields.dayOfWeek}-${fields.hourIndex}`, ...fields }));
    const auditMock = t.mock.method(auditLogRepository, 'createAuditLogEntry', async () => {});
    t.after(() => { findMock.mock.restore(); createMock.mock.restore(); auditMock.mock.restore(); });

    // 09:00-11:00, 60-min slots, break after every 1 slot -> Slot 1
    // (09:00-10:00), break (10:00-11:00 skipped), no room for Slot 2.
    const result = await academicService.generateSlotGrid({}, 'college-1', {
      workingDays: ['Monday', 'Tuesday'], startTime: '09:00', endTime: '11:00', slotDurationMinutes: 60, breakAfterSlots: 1,
...
test('generateTimetable — Theory/Practical/co-teaching/session blocks', async (t) => {
  ...
  function mockLookups(t, { periods = PERIODS, existingForClass = [], staffAllocations = [] } = {}) {
    const findClassMock = t.mock.method(classRepository, 'findById', async () => ({
      id: 'class-1', college_id: 'c1', timetable_status: 'Draft', max_hours_per_day_per_staff: null,
    }));
    ...
    const findByStaffMock = t.mock.method(facultyAllocationRepository, 'findByStaffUserId', async () => staffAllocations);
    const auditMock = t.mock.method(auditLogRepository, 'createAuditLogEntry', async () => {});
  ...
  await t.test('rejects Theory with two faculty', async (t2) => {
    ...
    await assert.rejects(
      () => academicService.generateTimetable({}, 'class-1', [
        {
          subject: 'Maths', subjectType: 'Theory', staffUserIds: ['s1', 's2'], periodsPerWeek: 1,
        },
      ]),
  ...
  await t.test('rejects Practical with three faculty', async (t2) => {
    ...
    await assert.rejects(
      () => academicService.generateTimetable({}, 'class-1', [
        {
          subject: 'Physics Lab', subjectType: 'Practical', staffUserIds: ['s1', 's2', 's3'], periodsPerWeek: 1,
        },
      ]),
      academicService.TimetableGenerationValidationError,
    );
  ...
  await t.test('rejects sessionBlocks that do not sum to periodsPerWeek', async (t2) => {
    ...
    await assert.rejects(
      () => academicService.generateTimetable({}, 'class-1', [
        {
          subject: 'Physics Lab', subjectType: 'Practical', staffUserIds: ['s1'], periodsPerWeek: 5, sessionBlocks: [2, 2],
        },
      ]),
      academicService.TimetableGenerationValidationError,
    );
  ...
  await t.test('places a two-faculty practical block as one continuous window, both faculty on every row', async (t2) => {
    const restore = mockLookups(t2);
    const createMock = t2.mock.method(facultyAllocationRepository, 'create', async (client, fields) => ({ id: `alloc-${fields.periodId}-${fields.staffUserId}`, ...fields }));
    t2.after(() => { restore(); createMock.mock.restore(); });

    const result = await academicService.generateTimetable({}, 'class-1', [
      {
        subject: 'Physics Lab', subjectType: 'Practical', staffUserIds: ['fac-1', 'fac-2'], periodsPerWeek: 2, sessionBlocks: [2],
      },
    ]);
  ...
  await t.test('max hours/day per staff blocks a placement that would exceed the cap', async (t2) => {
    ...
    const createMock = t2.mock.method(facultyAllocationRepository, 'create', async (client, fields) => ({ id: `alloc-${fields.periodId}`, period_id: fields.periodId, staff_user_id: fields.staffUserId, session_block_id: fields.sessionBlockId, ...fields }));
    t2.after(() => { restore(); createMock.mock.restore(); });
  ...
  await t.test('a staff-role actor who is not this class\'s tutor is forbidden', async (t2) => {
    ...
    t2.after(() => { restore(); resolveMock.mock.restore(); });
  ...
  await t.test('a staff-role actor who IS this class\'s tutor is still forbidden (Position Occupancy alone is not Current Login Identity)', async (t2) => {
    const restore = mockLookups(t2);
  ...
  await t.test('a class_tutor-effectiveRole actor (genuine Position Account seat login) who IS this class\'s tutor is allowed', async (t2) => {
    ...
    const resolveMock = t2.mock.method(identityService, 'resolvePositionOccupant', async () => 'tutor-1');
    const createMock = t2.mock.method(facultyAllocationRepository, 'create', async (client, fields) => ({ id: `alloc-${fields.periodId}`, period_id: fields.periodId, staff_user_id: fields.staffUserId, session_block_id: fields.sessionBlockId, ...fields }));
    t2.after(() => { restore(); resolveMock.mock.restore(); createMock.mock.restore(); });
  ...
  await t.test('a principal actor is never ownership-checked', async (t2) => {
    const restore = mockLookups(t2);
    const resolveMock = t2.mock.method(identityService, 'resolvePositionOccupant', async () => { throw new Error('should not be called'); });
    const createMock = t2.mock.method(facultyAllocationRepository, 'create', async (client, fields) => ({ id: `alloc-${fields.periodId}`, period_id: fields.periodId, staff_user_id: fields.staffUserId, session_block_id: fields.sessionBlockId, ...fields }));
    t2.after(() => { restore(); resolveMock.mock.restore(); createMock.mock.restore(); });
...
test('reviseTimetable', async (t) => {
  ...
  await t.test('removes only the changed subject\'s rows and leaves the other subject untouched', async (t2) => {
    const findClassMock = t2.mock.method(classRepository, 'findById', async () => ({
      id: 'class-1', college_id: 'c1', department_id: 'dept-1', timetable_status: 'Approved', max_hours_per_day_per_staff: null,
    }));
    const findPeriodsMock = t2.mock.method(timetablePeriodRepository, 'findAllByCollege', async () => PERIODS);
    const existingRows = [
      {
        id: 'alloc-maths', class_id: 'class-1', period_id: 'mon-1', subject: 'Maths', staff_user_id: 'old-fac',
      },
      {
        id: 'alloc-physics', class_id: 'class-1', period_id: 'mon-2', subject: 'Physics', staff_user_id: 'fac-2',
      },
    ];
    const removedIds = [];
    const findBySubjectMock = t2.mock.method(facultyAllocationRepository, 'findByClassAndSubject', async (client, classId, subject) => existingRows.filter((r) => r.subject === subject));
    const removeMock = t2.mock.method(facultyAllocationRepository, 'remove', async (client, id) => { removedIds.push(id); });
    const findByClassMock = t2.mock.method(facultyAllocationRepository, 'findByClassId', async () => existingRows.filter((r) => !removedIds.includes(r.id)));
    ...
    const resolveChainMock = t2.mock.method(workflowChainService, 'resolveApproverChain', async () => ['hod-1', 'principal-1']);
    const submitRequestMock = t2.mock.method(workflowService, 'submitRequest', async () => ({ id: 'wf-1', status: 'Pending' }));
    const updateClassMock = t2.mock.method(classRepository, 'update', async () => ({ id: 'class-1', timetable_status: 'Pending HOD' }));
    const auditMock = t2.mock.method(auditLogRepository, 'createAuditLogEntry', async () => {});
    t2.after(() => {
      findClassMock.mock.restore(); findPeriodsMock.mock.restore(); findBySubjectMock.mock.restore();
      removeMock.mock.restore(); findByClassMock.mock.restore(); findByStaffMock.mock.restore();
      createMock.mock.restore(); resolveChainMock.mock.restore(); submitRequestMock.mock.restore();
      updateClassMock.mock.restore(); auditMock.mock.restore();
    });

    const result = await academicService.reviseTimetable({}, 'class-1', [
