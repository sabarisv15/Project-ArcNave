...
function baseCfg() {
  return {
    projectId: 'proj', location: 'global', model: 'qwen/qwen3-next-80b-a3b-thinking-maas', accessToken: 'fake-token',
  };
}
...
function toolContext() {
  return aiContextAssembly.contextFromFlatPrompts({
    ...
    tools: [{ name: 'attendance_summary', description: 'reports attendance.', params: { type: 'object', properties: {} } }],
  });
}
...
test('completeWithMeta: a normal finish_reason "stop" answer is returned unchanged', async () => {
  await withStubbedFetch(async () => jsonResponse({
  ...
  await withStubbedFetch(async () => jsonResponse({
    choices: [{
      ...
      message: {
        content: null,
        tool_calls: [{
          id: 'call_1', type: 'function', function: { name: 'attendance_summary', arguments: '{"classId":"CSE-A"}' },
        }],
      },
    }],
  ...
  await withStubbedFetch(async () => jsonResponse({
    choices: [{
      ...
      message: {
        content: null,
        tool_calls: [{
          id: 'call_1', type: 'function', function: { name: 'attendance_summary', arguments: '{"classId":"CSE' },
        }],
      },
    }],
  }), async () => {
...
test('completeWithTools: a content-embedded tool-call fallback is never parsed/executed when finish_reason is "length"', async () => {
  await withStubbedFetch(async () => jsonResponse({
    choices: [{
      finish_reason: 'length',
      message: { content: '<tool_call>{"name":"attendance_summary","arguments":{"classId":"CSE', tool_calls: null },
    }],
  }), async () => {
    await assert.rejects(
      () => vertexMaas.completeWithTools(baseCfg(), toolContext()),
...
test('completeWithTools: an unrecognized finish_reason value does not reject an otherwise-valid tool call', async () => {
  await withStubbedFetch(async () => jsonResponse({
    choices: [{
      ...
      message: { content: null, tool_calls: [{ id: 'call_1', type: 'function', function: { name: 'attendance_summary', arguments: '{}' } }] },
    }],
  }), async () => {
    const result = await vertexMaas.completeWithTools(baseCfg(), toolContext());
    assert.equal(result.type, 'tool_call');
...
test('completeWithTools: finish_reason "content_filter" is never treated as a completed answer', async () => {
  await withStubbedFetch(async () => jsonResponse({
    choices: [{ finish_reason: 'content_filter', message: { content: 'partial before the filter tripped', tool_calls: null } }],
  }), async () => {
    await assert.rejects(
      () => vertexMaas.completeWithTools(baseCfg(), toolContext()),
...
test('completeWithTools: a native tool call keeps the provider\'s own ID unchanged', async () => {
  await withStubbedFetch(async () => jsonResponse({
    choices: [{
      ...
      message: {
        content: null,
        tool_calls: [{ id: 'call_provider_123', type: 'function', function: { name: 'get_attendance', arguments: '{"threshold":75}' } }],
      },
    }],
  }), async () => {
    const result = await vertexMaas.completeWithTools(baseCfg(), toolContext());
    ...
    const messages = vertexMaas.buildPriorTurnMessages([{
      toolName: result.toolName, arguments: result.arguments, callId: result.callId, rawToolCall: result.rawToolCall, resultText: 'ok',
    }]);
...
test('completeWithTools: a content-embedded tool call with no provider ID gets a non-empty local_ prefixed ID', async () => {
  await withStubbedFetch(async () => contentEmbeddedToolCallResponse('get_attendance', { threshold: 75 }), async () => {
    const result = await vertexMaas.completeWithTools(baseCfg(), toolContext());
    assert.equal(result.type, 'tool_call');
...
test('completeWithTools + buildPriorTurnMessages: the exact normalized callId from a content-embedded call reaches tool_call_id, and survives JSON.stringify/parse', async () => {
  ...
  const messages = vertexMaas.buildPriorTurnMessages([{
    toolName: 'get_attendance', arguments: { threshold: 75 }, callId, rawToolCall: undefined, resultText: '{"count":12}',
  }]);
  ...
  assert.ok(Object.prototype.hasOwnProperty.call(roundTripped[1], 'tool_call_id'), 'tool_call_id must survive JSON serialization, never be dropped');
