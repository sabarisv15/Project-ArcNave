...
same session as the RS-ACA-005 seat-login fix (commit `27f659b`).
...
| Timetable generation | ✅ `requireAuth` + ownership (RS-ACA-005) | *(hidden — correct, not Staff's role)* | ✅ `academic_generate_timetable`, `academic_revise_timetable` | Complete — **resolved** (Phase 2, `428d817`) |
...
**Staff summary**: 100% of intentional capabilities have a GUI entry point and matching AI tool. The three real findings from the original audit — `finance_record_payment` over-widening AI, `timetable_periods.generate_grid` over-widening GUI, and RS-ACA-005 having no AI tool — are all **resolved** (see commits `1dbfb8e`/`428d817` above).
...
| Timetable generation/revision (RS-ACA-005) | ✅ (this session, fixed for seat logins) | Timetable | ✅ `academic_generate_timetable`, `academic_revise_timetable` | Complete — **resolved** (Phase 2, `428d817`) |
...
1. ~~RS-ACA-005 (timetable generation/revision) has zero AI tool coverage, for every role.~~ **Resolved** (Remediation Phase 2, commit `428d817`): `academic_generate_timetable`/`academic_revise_timetable` added to `aiToolRegistry.js`, thin wrappers over the existing `generateTimetable`/`reviseTimetable` ownership checks.
