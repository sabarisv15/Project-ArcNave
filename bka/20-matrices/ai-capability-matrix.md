...
| Tool register coverage | **Stale again, worse this time (flagged 2026-08-21, not re-derived).** Section 4 was last regenerated 2026-07-26 against 32 real tools. Root `CHECKPOINT.md`/`CHANGES.md` (rounds 13–18, commits `34914e5`…`578dc3f`) record ~62 tools now registered in `aiToolRegistry.js` plus whole new subsystems this section doesn't mention at all: conversation memory, the bounded multi-step workflow engine (`run_workflow_plan`), evidence/provenance + numeric-claim verification, real SSE streaming, token/cost telemetry, model routing, `fetch_trusted_web_page` (Trusted Web Retrieval), scoped preference memory (`user_preferences_set` restricted allowlist), `generate_document`/`export_artifact`/`update_artifact_content`, and a General/Curriculum tool-visibility split (`askGeneralChat` offers **zero** ARCNAVE tools by design). None of this has been run through the amendment procedure (Decision Ledger entry + `RS-AIG` edit + matrix regeneration) yet — this row only records that the gap exists, it does not close it. A real regeneration pass needs to re-derive Section 4 from the current `aiToolRegistry.js`, not from this note. |
| Provider register | **Not tracked here at all.** `ADR-028` still names NVIDIA NIM as sole production provider; the codebase now has 5 live-verified adapters (nim/gemini-via-Vertex-AI/claude/openai/self_hosted) selectable per college via `college_ai_config` or globally via `DEFAULT_AI_PROVIDER` (defaults to `nim`, unchanged). Whether "production provider" should now read as "nim, default" versus "configurable, multi-provider" is a product decision for whoever owns `ADR-028`, not something resolved by this note. Flagged 2026-08-21. |
