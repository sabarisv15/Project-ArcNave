...
|---|---|---|
| x-column-boundary detection | FUTURE — **the thing that would lift partial trust** | Correct column attribution needs detecting the printed column boundaries by x, which was done **by hand** during ADL-055's analysis, never automatically. Until it exists, partial trust is permanent for this path. Its own pass, and the pass that would then re-open the operations refused here. |
