            ...
            <>
              <div className="flex items-start justify-between gap-[12px] pt-[18px] px-[18px] pb-[14px] border-b border-line">
                ...
                <Dialog.Close asChild>
                  <button
                    type="button"
                    aria-label="Close class log detail"
                    title="Close"
              ...
              <Dialog.Description className="sr-only">
                Class log and attendance detail for {period.subject} on {formatFullDate(period.date)}.
              </Dialog.Description>
              ...
              <div className="flex-1 min-h-0 overflow-y-auto scroll-quiet py-[16px] px-[18px]">
                ...
                {canEdit ? (
                  ...
                  <div className="mt-[16px] pt-[14px] border-t border-line">
                    <div className="text-[11px] font-[500] uppercase tracking-[.06em] text-ink-faint mb-[8px]">
                      Class log
                    </div>
                    <div className="text-[13px] leading-[1.5] text-ink whitespace-pre-wrap">
                ...
                {session.lockedAt && (
                  <p className="mt-[12px] mb-0 text-[11.5px] text-ink-faint">
                    Locked {formatTime(session.lockedAt)} by {session.lockedBy}.
                  </p>
                )}
                {session.submittedAt && (
                  <p className="mt-[4px] mb-0 text-[11.5px] text-ink-faint">
                    Submitted {formatTime(session.submittedAt)} by {session.submittedBy}. Included in attendance
                    percentage.
                  </p>
                )}
                {!session.lockedAt && (
                  <p className="mt-[12px] mb-0 text-[11.5px] text-ink-faint">
                    Still a draft. Not locked, and not yet visible in the Class login.
                  </p>
                )}
              </div>
