...
export function ClassLogSortControl({ c }) {
  return (
    <div className="relative">
      ...
      {c.sortMenuOpen && (
        <div role="menu" className="absolute top-[calc(100%+6px)] left-0 z-[60] min-w-[180px] p-[5px] bg-raised border border-line-strong rounded-[16px] shadow-pop animate-fadeUp">
          {CLASS_LOG_SORTS.map((opt) => {
            ...
            return (
              <button
                ...
                type="button"
                role="menuitem"
                onClick={() => { c.setSortKey(opt.key); c.setPanel(null); }}
                className={cn(
...
export function ClassLogChips({ chips, onRemove, onClearAll }) {
  ...
  return (
    <div className="flex-none flex flex-wrap gap-[8px] mb-[12px] animate-fadeUp">
      {chips.map((chip) => (
        <span key={chip.key} className="inline-flex items-center gap-[4px] h-[28px] pl-[11px] pr-[4px] bg-accent-soft border border-accent-line rounded-[11px] text-[12px] text-accent">
      ...
      <button type="button" onClick={onClearAll} className="h-[28px] px-[10px] border-0 bg-transparent rounded-[10px] font-sans text-[12px] text-ink-muted cursor-pointer transition-colors duration-200 hover:bg-tint2 hover:text-ink">
        Clear all
      </button>
    </div>
  );
  ...
  return (
    ...
    >
          ...
          {DATE_PRESETS.map((p) => (
            <button
              ...
              className={cn(
                ...
                c.datePreset === p.key ? 'bg-accent-soft border border-accent-line text-accent font-[600]' : 'bg-tint2 border border-transparent text-ink-soft font-[500] hover:bg-hoverline'
              )}
            >
              {p.label}
            </button>
        ...
        {c.datePreset === 'custom' && (
          <div className="grid grid-cols-2 gap-[8px] mt-[8px]">
            <FilterField label="From">
              <input type="date" aria-label="Custom range from" value={c.customFrom} onChange={(e) => c.setCustomFrom(e.target.value)} className={FILTER_FIELD_INPUT} />
            </FilterField>
            <FilterField label="To">
              <input type="date" aria-label="Custom range to" value={c.customTo} onChange={(e) => c.setCustomTo(e.target.value)} className={FILTER_FIELD_INPUT} />
            </FilterField>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 gap-x-[12px] gap-y-[14px]">
        <FilterSelect label="Ownership" value={f.ownership} onChange={(v) => set('ownership', v)} options={[{ value: '', label: 'All' }, { value: 'own', label: 'My class' }, { value: 'substitute', label: 'Substitute duty' }]} />
        <FilterSelect label="Class / section" value={f.code} onChange={(v) => set('code', v)} options={opts('All classes', CODES)} />
        <FilterSelect label="Subject" value={f.subject} onChange={(v) => set('subject', v)} options={opts('All subjects', SUBJECTS)} />
