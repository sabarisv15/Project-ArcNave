...
import { ACADEMIC_OPTIONS, ATTENDANCE_OPTIONS, SCOPE_BATCHES, SCOPE_DEPTS, SCOPE_SECTIONS } from '../lib/studentsData';
...
const opts = (all, list, map = (v) => v) => [
  { value: '', label: all },
  ...list.map((v) => ({ value: v, label: map(v) })),
];
    ...
    <div className="bg-tint border border-line rounded-[16px] pt-[16px] px-[18px] pb-[18px] mb-[12px] animate-fadeUp">
      ...
      <div className="grid grid-cols-[repeat(auto-fit,minmax(168px,1fr))] gap-x-[18px] gap-y-[14px]">
        ...
        <Select
          ...
          options={opts('All sections', SCOPE_SECTIONS, (v) => `Section ${v}`)}
        />
        <Select
        ...
        <Select
          ...
          options={opts('All batches', SCOPE_BATCHES)}
        />
        <Select
          label="Entry type"
          value={f.entry}
          onChange={(v) => set('entry', v)}
          options={opts('All', ['Regular', 'Lateral'])}
        />
        <Select
          label="Accommodation"
          value={f.accom}
          onChange={(v) => set('accom', v)}
          options={opts('All', ['Hosteller', 'Day Scholar'])}
        />
        <Select
          label="Gender"
          value={f.gender}
          onChange={(v) => set('gender', v)}
          options={opts('All', ['Male', 'Female'])}
        />
      </div>
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(168px,1fr))] gap-x-[18px] gap-y-[14px] mt-[12px] animate-fadeUp">
          <Field label="Min CGPA">
            <input
              ...
              value={f.cgpaMin}
              onChange={(e) => set('cgpaMin', e.target.value)}
              placeholder="e.g. 7.7"
              className={FIELD}
            />
          </Field>
          <Field label="Backlogs ≥">
            <input
              ...
              step="1"
              min="0"
              aria-label="Minimum backlogs"
              value={f.arrearsMin}
              onChange={(e) => set('arrearsMin', e.target.value)}
              placeholder="e.g. 9"
              className={FIELD}
            />
          </Field>
          <Field label="Backlogs <">
            <input
              ...
              step="1"
              min="0"
              aria-label="Maximum backlogs"
              value={f.arrearsMax}
              onChange={(e) => set('arrearsMax', e.target.value)}
              placeholder="e.g. 3"
              className={FIELD}
            />
          ...
          <Field label="Attendance % ≥">
            <input
              ...
              value={f.attMin}
              onChange={(e) => set('attMin', e.target.value)}
              placeholder="e.g. 90"
              className={FIELD}
            />
          </Field>
          <Field label="Attendance % <">
            <input
              ...
              value={f.attMax}
              onChange={(e) => set('attMax', e.target.value)}
              placeholder="e.g. 50"
              className={FIELD}
            />
          </Field>
