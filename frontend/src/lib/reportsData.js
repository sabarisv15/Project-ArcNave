...
const CLASS_DEFS = [
  {
    ...
    rosterCount: 45,
    topics: [
      'Arrays and pointers: memory layout',
      'Singly linked lists: insertion and deletion',
      'Stacks: array vs linked implementation',
      'Queues and circular queues',
      'Binary search trees: insertion and traversal',
      ...
      'Sorting: merge sort and complexity',
      'Sorting: quick sort and pivot strategies',
    ],
  },
  {
    code: 'III B.Sc CS — A',
    ...
    rosterCount: 40,
    topics: [
      'ER modelling: entities and relationships',
      'Relational model and keys',
      'SQL joins: inner, outer, and self joins',
      'Normalization: 1NF through 3NF with worked examples',
      'Transactions and ACID properties',
      'Concurrency control: locking basics',
      'Indexing: B-tree structures',
  ...
  },
  {
    ...
    topics: [
      ...
      'Network security basics: firewalls and encryption',
    ],
  },
  {
    ...
    rosterCount: 32,
    topics: [
      'Wave optics: interference and Young’s double slit',
      'Refraction through prisms',
      'Electrostatics: Coulomb’s law',
      'Current electricity: Ohm’s law and resistivity',
...
function buildClassSessions(def) {
  ...
  while (cursor < today) {
    if (def.weekdays.includes(istWeekday(cursor))) {
      ...
      const presentIds = new Set(roster.filter((s) => presenceRnd() < tendency.get(s.id)).map((s) => s.id));
