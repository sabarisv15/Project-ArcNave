...
import {
  ASSESSMENT_SORTS, ASSESSMENT_TYPES, TYPE_LABELS, eligibleScopes, marksProgress, scopeById, studentsForScope,
...
const GRID = 'grid grid-cols-[minmax(0,1.5fr)_minmax(0,130px)_minmax(0,1.1fr)_minmax(0,1fr)_minmax(0,104px)_minmax(0,124px)_minmax(0,96px)] gap-x-[12px] items-center px-[16px]';

/** Draft / Published — the two states, carried by the word, reinforced by a dot. */
function StatusText({ status }) {
  const published = status === 'published';
  return (
    <span className={cn('inline-flex items-center gap-[6px] text-[12px] font-[500]', published ? 'text-success' : 'text-ink-muted')}>
      <span className={cn('flex-none w-[6px] h-[6px] rounded-full', published ? 'bg-success' : 'bg-ink-disabled')} aria-hidden="true" />
...
function useAssessmentList() {
  ...
  return {
    rows, total: assessments.length,
    query, setQuery, sortKey, setSortKey,
    filtersOpen, setFiltersOpen, filters, setFilter, clearFilters,
  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col overflow-hidden px-[28px] pt-[22px] pb-[20px] animate-viewIn">
      <div className="flex-none flex items-center gap-[10px] mb-[12px]">
        ...
        />
        <SortIconPopover options={ASSESSMENT_SORTS} value={l.sortKey} onChange={l.setSortKey} label="Sort assessments" />
        <FilterPopover
          open={l.filtersOpen}
        ...
        >
          <div className="grid grid-cols-1 gap-[14px]">
            ...
            <FilterSelect
              ...
              options={[{ value: '', label: 'All' }, ...scopes.map((s) => ({ value: s.id, label: `${s.subject} · ${s.code}` }))]}
            />
            <FilterSelect
              label="State"
      ...
      <div className="flex-1 min-h-0 border border-line rounded-[16px] bg-paper overflow-hidden">
        <div className="h-full overflow-auto scroll-quiet">
          <div className="min-w-[980px]">
            <div className={cn(GRID, TABLE_HEAD, 'sticky top-0 z-[46] h-[36px] bg-paper shadow-[inset_0_-1px_0_theme(colors.line.DEFAULT)]')}>
            ...
            {l.rows.map((a) => {
              ...
              return (
                <div
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setOpenId(a.id); }
                  }}
                  className={cn(GRID, 'h-[46px] border-t border-line-light cursor-pointer transition-colors duration-200 hover:bg-tint2 outline-none focus-visible:bg-tint2')}
                >
                  <span className="min-w-0 text-[13px] font-[500] text-ink truncate" title={a.name}>{a.name}</span>
                  <span className="min-w-0 text-[12px] text-ink-muted truncate">{TYPE_LABELS[a.type]}</span>
                  <span className="min-w-0 text-[12.5px] text-ink truncate" title={a.subject}>{a.subject}</span>
                  <span className="min-w-0 text-[12.5px] text-ink-muted truncate" title={a.code}>{a.code}</span>
                  <span className="text-[12.5px] text-ink-muted tabular-nums whitespace-nowrap">{formatDateDMY(a.date)}</span>
                  ...
                  <span><StatusText status={a.status} /></span>
                </div>
              );
            })}
            ...
            {l.rows.length === 0 && (
              <TableEmptyState
                ...
                hint={l.total ? 'Try clearing a filter or search term.' : 'Use Create assessment to add one for a class you teach.'}
              />
            )}
          </div>
        </div>
      ...
      <AssessmentCreateDrawer open={createOpen} onClose={() => setCreateOpen(false)} onCreated={(a) => setOpenId(a.id)} />
